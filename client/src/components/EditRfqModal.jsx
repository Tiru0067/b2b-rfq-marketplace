...
import { updateRfq } from '../api/rfqApi';

const EditRfqModal = ({ isOpen, onClose, rfq, onRfqUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateRfq(rfq.id, payload);
      if (res.success) {
        onRfqUpdated(res.data);
        onClose();
      }
    } catch (err) {
      setError(
        err.response?.data?.message || 'Failed to update RFQ. Please check all fields.'
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl border border-border-default overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-default">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-typography-900">Edit RFQ</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-border-default">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm font-semibold text-typography-700 hover:bg-slate-100 rounded-xl cursor-pointer"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 px-5 py-2 bg-primary-600 hover:bg-primary-700 disabled:opacity-50 text-white text-sm font-semibold rounded-xl shadow-sm transition-colors cursor-pointer"
            >
